...
public class MM597PiceCleanProcessor extends Thread{
	...
	public static void main(String[] args) {
		AppRun.start();
		MM597PiceCleanProcessor a = new MM597PiceCleanProcessor("mm597_price");
		a.start();
	}
	...
	public void run() {
		...
		System.out.println("-------------597苗木【待清洗数据"+count+"条】-------------");
		if(count>0){
			...
			for (int i=degree,j=0;i>j;j++) {
				...
				System.out.println("597苗木【已同步数据："+savaDate+"条,剩余"+(count-savaDate)+"条数据】");
			}
			System.out.println("597苗木【已清洗数据："+ids.size()+"条】");
			//更改爬取数据状态（线程）
			ContentProcessor ContentProcessor = new ContentProcessor(code,ids);
			ContentProcessor.start();	
			System.out.println("-------------597苗木【成功保存进price_product："+savaDate+"条】-------------");	
		}else{
			System.out.println("-------------597苗木【无数据保存进price_product】-------------");	
		}
	}
}

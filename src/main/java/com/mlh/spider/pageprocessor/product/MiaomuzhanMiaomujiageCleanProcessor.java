 ...
 * 第一苗木 清洗处理器
 */
public class MiaomuzhanMiaomujiageCleanProcessor extends Thread{
	
	private String code;
	
	public MiaomuzhanMiaomujiageCleanProcessor(String code) {
		this.code=code;
	}
	...
	public void run() {//miaomuzhan_miaomujiage
		...
		if(count>0){
			...
			for (int i=degree,j=0;i>j;j++) {
				...
				System.out.println("第一苗木【已同步数据："+savaDate+"条,剩余"+(count-savaDate)+"条数据】");
			}
			System.out.println("第一苗木【已清洗数据："+ids.size()+"条】");
			//更改爬取数据状态（线程）
			ContentProcessor ContentProcessor = new ContentProcessor(code,ids);
			ContentProcessor.start();	
			System.out.println("-------------第一苗木【成功保存进price_product："+savaDate+"条】-------------");	
		}else{
			System.out.println("-------------第一苗木【无数据保存进price_product】-------------");	
		}
	}
}
